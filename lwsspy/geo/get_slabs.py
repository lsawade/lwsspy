...
def get_slabs(key='dep'):
    ...
    tarname = os.path.join(lpy.DOWNLOAD_CACHE, 'slabs.tar.gz')
    dirname = os.path.join(lpy.DOWNLOAD_CACHE, 'slabs')
    ...
    if not os.path.exists(dirname):
        ...
        if not os.path.exists(tarname):
            lpy.downloadfile(url, tarname)

        # Then, either way, unzipping has to be done
        lpy.untar(tarname, dirname)
