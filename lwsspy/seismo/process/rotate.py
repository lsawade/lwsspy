    ...
    if len(input_components) == 2:
        ...
        for i_1, i_2 in zip(input_1, input_2):
            ...
            if azi1 is None or azi2 is None:
                bad_ids.extend([i_1.id, i_2.id])
                print("Unable to rotate [%s, %s] due to: "
                      "Unable to extract channel orientation information."
                      % (i_1.id, i_2.id))
                continue
            output_1, output_2 = rotate_12_rt(i_1.data, i_2.data, back_azimuth,
                                              azi1, azi2)
            # Check whether components are orthogonal
            if output_1 is None or output_2 is None:
                bad_ids.extend([i_1.id, i_2.id])
                print("Unable to rotate [%s, %s] due to: "
                      "Components to be rotated not orthogonal."
                      % (i_1.id, i_2.id))
                continue
...
def rotate_stream(st, event_latitude, event_longitude,
    ...
    for sta_stream in sorted_st_dict.values():
        ...
        if len(sta_stream) == 2:
            if all([_tr.stats.component in ["N", "E"] for _tr in sta_stream]):
                pass
            elif all([_tr.stats.component in ["1", "2"] for _tr in sta_stream]):
                pass
            else:
                ids = [_tr.id for _tr in sta_stream]
                print(f"Unable to rotate [{ids[0]}, {ids[1]}] due to: "
                      f"Only one horizontal component available. "
                      "Attaching vertical only.")
                rotated_stream += sta_stream.select(component="Z")
                continue

        elif len(sta_stream) == 1:

            if sta_stream[0].stats.component == 'Z':
                print(f"Unable to rotate [{sta_stream[0].id}] due to: "
                      f"Only vertical component available. Attached.")
                rotated_stream += sta_stream.select(component="Z")
                continue
            else:
                print(f"Unable to rotate [{sta_stream[0].id}] due to: "
        ...
        _st = rotate_one_station_stream(sta_stream, event_latitude,
